...
namespace RtShogi.Scripts.Battle
{
    ...
    public static class KomaFormingChecker
    {
        ...
        {
            if (nextPoint.Raw.Z >= BoardMap.BoardSize.H - 1) return EKomaFormAble.FormForced;
            if (nextPoint.Raw.Z >= BoardMap.BoardSize.H - 3) return EKomaFormAble.FormAble;
            return EKomaFormAble.Impossible;
        }
        private static EKomaFormAble canFormOfKeima(ImBoardPoint nextPoint)
        {
            if (nextPoint.Raw.Z >= BoardMap.BoardSize.H - 2) return EKomaFormAble.FormForced;
            if (nextPoint.Raw.Z >= BoardMap.BoardSize.H - 3) return EKomaFormAble.FormAble;
            return EKomaFormAble.Impossible;
        }
        private static EKomaFormAble canFormOfHishaOrKakuOrGin(ImBoardPoint currPoint, ImBoardPoint nextPoint)
        {
            if (currPoint.Raw.Z >= BoardMap.BoardSize.H - 3 || nextPoint.Raw.Z >= BoardMap.BoardSize.H - 3) 
